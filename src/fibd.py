...
wabbits = np.zeros(n, dtype='object')  # use python number to avoid integer overflow
wabbits[0] = 1  # start with 1 pair of rabbit in month 1

rates = np.zeros((n, n), dtype='object')
for i in xrange(m - 1):
    rates[i, i + 1] = 1  # rabbits do not survive beyond m month

rates[1:n, 0] = k  # after 2 months, rabbits start reproducing
